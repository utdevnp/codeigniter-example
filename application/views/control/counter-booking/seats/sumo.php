              ...
              <div class="col-md-7 bus-seats">
                ...
                <div class="tot-cabin-hice" style="margin-bottom: 2px;">
                 ...
                 <?php  
                      $i=1;   
                      while($i<=2){  ?>
                        <div  class="select col-md-1 <?php if($bookedseat!==""){ for($m=0;$m<$all;$m++){ $myside = sscanf($bookedseat[$m], "%[A-Z]%d"); if($myside[0]=="F"){  if($myside[1]%2==0 AND $myside[1]==$i){ echo "disconnect";} }}} if(@$reservetot>0){for($n=0;$n<$reservetot;$n++){$resv = sscanf(@$expres[$n], "%[A-Z]%d"); if($resv[0]=="F"){ if($resv[1]==$i){ echo "disconnect";}}}}?>" price="<?php echo $price;?>" title="<?php echo $everybus;?>" id="F<?php echo $i;?>"><span class="seat-no"><?php  echo 'F';?><?php echo $i;?></span></div>
                          <?php $i = $i+1; 
                      } ?>
                 </div>
